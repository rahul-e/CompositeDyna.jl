...
#    srxy::Float64=0.0
    ...
    sfdj = fill(0.0, (2, 8))
    b = zeros(Float64, 8, 40)
    db = zeros(Float64, 8, 40)
        ...
        for iy=1:2
            ...
            for i=1:8
                ...
                if(i<=4)
                    ...
                    aa+=(r[i]^2-1.0)*GP[ix]^2
                    bb+=(s[i]^2-1.0)*GP[iy]^2
                    sf[i]=0.5*aa*bb
            ...
            for i=1:8
                for j=1:2
                    xjac[j,1]+=xl[i]*sfd[j,i]
                    xjac[j,2]+=yl[i]*sfd[j,i]
    ...
    m,n=size(ek)
    println("Size of EK is ", m,"×",n)
    println("Diagonal elements of EK matrix:")
    open("EK.txt", "w") do file
        for i in 1:40
            for j in 1:40
                   println(file, ek[i,j])
            end
